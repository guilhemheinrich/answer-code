...
dashboardPage(
  dashboardHeader(title = "Model App"),
  dashboardSidebar(
    selectInput(inputId = "experiment",
                label = "Choose an experiment:",
                choices = setNames(unique(getExperiments2()$data$uri), unique(getExperiments2()$data$alias))[7] #,
  ...
  dashboardBody(
    fluidRow(plotlyOutput("distPlot") %>% withSpinner(color="#0dc5c1")),
    uiOutput("models") %>% withSpinner(color="#0dc5c1"), uiOutput("style")
  )
)
